...
import { Images, relativeTo, sourceImages, } from "../helpers.js";
...
export class AndroidPlatform extends Platform {
    constructor(options, logger) {
        super(options, uniformIconOptions(options, options.icons.android, ICONS_OPTIONS), logContext(logger, "android"));
    }
    async createImages(sourceset) {
        const images = new Images(this.log);
        let icons = await Promise.all(Object.entries(this.iconOptions).map(([iconName, iconOption]) => images.createFavicon(sourceset, iconName, iconOption)));
        ...
        if (this.options.manifestMaskable &&
            ...
            this.log("General:source", `Maskable source type is ${typeof this.options.manifestMaskable}`);
            const maskableSourceset = await sourceImages(this.options.manifestMaskable);
            const maskable = await Promise.all(Object.entries(ICONS_OPTIONS_MASKABLE).map(([iconName, iconOption]) => images.createFavicon(maskableSourceset, iconName, iconOption)));
    ...
    manifestFileName() {
        var _a, _b, _c;
        return (_c = (_b = (_a = this.options.files) === null || _a === void 0 ? void 0 : _a.android) === null || _b === void 0 ? void 0 : _b.manifestFileName) !== null && _c !== void 0 ? _c : "manifest.json";
    }
    manifest() {
        const { options } = this;
        ...
        properties.icons = Object.entries(icons).map(([name, iconOptions]) => {
            ...
            return {
                ...
                type: "image/png",
                purpose: (_a = iconOptions.purpose) !== null && _a !== void 0 ? _a : defaultPurpose,
            };
        });
        return {
