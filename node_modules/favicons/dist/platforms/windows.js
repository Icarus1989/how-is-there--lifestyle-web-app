...
export class WindowsPlatform extends Platform {
    constructor(options, logger) {
        super(options, uniformIconOptions(options, options.icons.windows, ICONS_OPTIONS), logContext(logger, "windows"));
    }
    async createFiles() {
    ...
    browserConfig() {
        ...
        for (const { name, ...size } of SUPPORTED_TILES) {
            const icon = Object.entries(this.iconOptions).find((icon) => hasSize(size, icon[1]));
            if (icon) {
                tile[name] = {
